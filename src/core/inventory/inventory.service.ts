...
export class InventoryService {
  ...
  async createInventory(createInventoryDto: CreateInventoryDto, adminId: number) {
    const admin = await this.prismaService.user.findUnique({
      where: { id: adminId, role: RoleEnum.ADMIN },
    });
    if (!admin) throw new ForbiddenException('Only admins can create inventories');

    return this.inventoryRepo.createInventory({
      ...createInventoryDto,
    });
  }

  async updateInventory(id: number, updateInventoryDto: UpdateInventoryDto, adminId: number) {
    const admin = await this.prismaService.user.findUnique({
      where: { id: adminId, role: RoleEnum.ADMIN },
    });
    if (!admin) throw new ForbiddenException('Only admins can update inventories');

    const inventory = await this.inventoryRepo.getInventoryById(id);
    if (!inventory) throw new NotFoundException('Inventory not found');

    return this.prismaService.$transaction(async (prisma) => {
      const { name, initialBalance } = updateInventoryDto;
  ...
  async deleteInventory(id: number, adminId: number) {
    const admin = await this.prismaService.user.findUnique({
      where: { id: adminId, role: RoleEnum.ADMIN },
    });
    if (!admin) throw new ForbiddenException('Only admins can delete inventories');
  ...
  async getInventories(inventoryQueryDto: InventoryQueryDto, user: any) {
    const { page = 1, limit = 20, name, minBalance, maxBalance, ...filter } = inventoryQueryDto;
    ...
    const inventories = await this.prismaService.inventory.findMany({
      ...
      take,
      skip,
      include: { workers: true, products: { include: { product: true } } },
    });
  ...
  async getInventoryByUserId(userId: number) {
    const user = await this.prismaService.user.findUnique({
      where: { id: userId },
      include: { inventory: { include: { products: { include: { product: true } }, workers: true } } },
    });
    if (!user?.inventory) throw new NotFoundException('No inventory assigned');
    return user.inventory;
  }

  async addProductToInventory(id: number, addProductDto: AddProductDto, adminId: number) {
    const admin = await this.prismaService.user.findUnique({
      where: { id: adminId, role: RoleEnum.ADMIN },
    });
    if (!admin) throw new ForbiddenException('Only admins can add products');
  ...
  async addWorkerToInventory(id: number, addWorkerDto: AddWorkerDto, adminId: number) {
    const admin = await this.prismaService.user.findUnique({
      where: { id: adminId, role: RoleEnum.ADMIN },
    });
