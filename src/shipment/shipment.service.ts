...
export class ShipmentService {
  ...
  async createShipment(createShipmentDto: CreateShipmentDto, user: User): Promise<ShipmentResponseDto> {
    ...
    const shipment = await this.shipmentRepo.createShipment(shipmentData);

    if (createShipmentDto.shipmentExpenses?.length) {
      await this.prismaService.shipmentExpense.createMany({
        data: createShipmentDto.shipmentExpenses.map(expense => ({
          shipmentId: shipment.id,
          name: expense.name,
          amount: expense.amount,
          description: expense.description,
          tag: expense.tag ? (Object.values(ExpenseTag).includes(expense.tag as any) ? expense.tag as ExpenseTag : ExpenseTag.OTHER) : ExpenseTag.OTHER,
        })),
      });
    }
  ...
  async updateShipment(id: number, updateShipmentDto: UpdateShipmentDto, user: User): Promise<ShipmentResponseDto> {
    ...
    if (user.role !== 'ADMIN') {
      throw new UnauthorizedException('Only admins can update shipment status');
    }
    ...
    if (shipment.status === StatusShipmentEnum.PENDING && updateShipmentDto.status === StatusShipmentEnum.PENDING_REVIEW) {
      ...
      if (updateShipmentDto.reviewMessage) {
        console.log(`Admin review note: ${updateShipmentDto.reviewMessage} for shipment ${id}`);
      }
      ...
      if (updateShipmentDto.isWaitingForChanges) {
        ...
        if (updateShipmentDto.reviewMessage) {
          console.log(`Admin returned shipment ${id} to PENDING with feedback: ${updateShipmentDto.reviewMessage}`);
        }
      } else if (updateShipmentDto.status === StatusShipmentEnum.ACCEPTED) {
  ...
  async getWeeklySummary(): Promise<any> {
    ...
    weeklyShipments.forEach(s => {
      ...
      weeklyTotalExpenses += s.shipmentExpenses.reduce((sum, exp) => sum + exp.amount, 0);
    });

    const weeklyNetCash = weeklyExpectedCash - weeklyTotalExpenses;
    const weeklyCashAfterDeposit = weeklyExpectedCash - (weeklyExpectedCash * 0.8);
